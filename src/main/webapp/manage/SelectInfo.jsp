                ...
                for (var i=0;i<data.list.length;i++)
                 {
                     var table="<tr><td>"+data.list[i].softwarename+"</td>+" +
                         ...
                         "<button type='button' class=\"layui-btn layui-btn-primary\" onclick='button1(this)' style='width: 90px'>审核<i class='layui-icon layui-icon-triangle-d'></i></button>" +
                         "<ul style='display: none'>" +
                         "<li><a style='width: 90px' class=\"layui-btn layui-btn-normal\" onclick=updateStatus("+data.list[i].id+",2)>通过</a></li>" +
                         "<li><a style='width: 90px' class=\"layui-btn layui-btn-normal\" onclick=updateStatus("+data.list[i].id+",3)>不通过</a></li>" +
                         "</ul></div></div></td></tr>"
                     $("#table1").append(table) ;
